    ...
    if (verbIndex !== -1 && ((token.pos === 'RP' && (!next || next.pos !== 'IN')) || token.pos === 'IN')) {
      ...
      if (phrasalVerbTokens.length > 1) {
        ...
        if (definition) {
          ...
          phrasalVerbTokens = await tokensAddFrequency(phrasalVerbTokens);
          phrasalVerbTokens = await tokensIsStopWord(phrasalVerbTokens);
          phrasalVerbTokens = await tokensAddDefinition(phrasalVerbTokens);
          phrasalVerbTokens = await tokensAddRank(phrasalVerbTokens);
